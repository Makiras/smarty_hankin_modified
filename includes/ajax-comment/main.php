...
if (!function_exists('fa_ajax_comment_scripts')) :
    ...
    function fa_ajax_comment_scripts()
    {
        ...
        wp_enqueue_style('ajax-comment', "https://cdn.jsdelivr.net/gh/Makiras/Makiras-Static@master/smarty_hankin" . '/includes/ajax-comment/app.css', array(), AC_VERSION);
        wp_enqueue_script('ajax-comment', "https://cdn.jsdelivr.net/gh/Makiras/Makiras-Static@master/smarty_hankin" . '/includes/ajax-comment/app.js', array('jquery'), AC_VERSION, true);
...
if (!function_exists('fa_ajax_comment_callback')) :
    ...
    function fa_ajax_comment_callback()
    {
        ...
        $commnet_id = get_comment_ID();
        $comment_author = get_comment_author($commnet_id);
        $comment_email = get_comment_author_email($commnet_id);
        $comment_author_url = get_comment_author_url($commnet_id);
        $comment_avatar_url = get_avatar_url($comment_email);
?>
        <li class="comment odd alt thread-odd thread-alt depth-1" id="li-comment-<?=$commnet_id?>">
            <article id="div-comment-<?=$commnet_id?>" class="comment-body d-flex flex-fill ">
                <div class="comment-avatar-author vcard mr-2 mr-md-3 ">
                    <div class="w-48">
                        <img src="<?=$comment_avatar_url ? "https://www.gravatar.com/avatar/" . hash('md5', $comment_email) . "?s=48&d=identicon" : $comment_avatar_url ?>" style="border-radius: 5px;">
                    </div>
                </div>
                    ...
                    <div class="comment-info d-flex align-items-center mb-1">
                        <div class="comment-author text-sm">
                            <?php if (!$comment_author_url) : ?>
                                <?= $comment_author ?>
                            <?php else : ?>
                                <a href="<?=$comment_author_url?>" target="_blank" rel='nofollow'><?=$comment_author ?></a>
                            <?php endif; ?>
